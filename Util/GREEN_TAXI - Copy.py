...
#df["Date"]=pd.to_datetime(df["Date"], infer_datetime_format=True,format='%Y-%m')
...
def read_Json(path="./ZIP_CODE.json"):
...
def Newyour_ZIP(path="./New_your_zip.csv"):
    ...
    def plot(self):
        ...
        fig.add_trace(
           go.Indicator(mode="gauge+number", value=self.Toal_trip, 
              title={'text': "Total Trip on FEB {day} at {HR}:00 ".format(
                  day=self.Daily_Hour,HR=self.Daily_Hour)},number={"font":{"size":25}}),
            row=3, col=1)
        ...
        fig.add_trace( go.Indicator(mode="gauge+number", value=self.Toal_Disctance,
              title={'text': "Total Traveled Distance on FEB {day} at {HR}:00  KM".format(
                ...
                # traces with separate domains to form a subplot
        fig.add_trace(go.Indicator(mode="gauge+number", value=self.Toal_trip, 
                title={'text': "Total Trip on FEB {day} at {HR}:00 ".format(
                    day=self.Daily_Hour,HR=self.Daily_Hour)},number={"font":{"size":25}}),
                     row=7, col=1) 
